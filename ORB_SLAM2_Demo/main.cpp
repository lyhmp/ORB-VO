...
//ORB
//#include ""System.h"
#include "../../../../../../../github/ORB_SLAM2/src/System.h"
...
void Track(cv::Mat& iRGB, cv::Mat& iDepth)
{
    double dTimeStamp = EGetCurTime();
...
void watcher(cv::Mat& iLast, cv::Mat& iTrans)
{
    cv::Mat iTemp = iTrans * iLast.inv();
...
void Modified(bool bRestart)
{
    ...
    if(bRestart)
    {
        ORB_SLAM2::System SLAM("ORBvoc.txt","Realsense.yaml",ORB_SLAM2::System::RGBD,true,true, "NULL");
        cout << "Init Realsense ...\n" << endl;
        //init realsense
        int nFrameHeight = 480;
        int nFrameWidth = 640;
    ...
    //        std::cout << setprecision(1) << std::fixed << ", fov = " << intrin.hfov() << " x " << intrin.vfov() << ", distortion = " << intrin.model() << std::endl;
        ...
        while(1)
        {
            ...
            cv::Mat iTrans = SLAM.TrackRGBD(ColorImage8bit,idepth_image,EGetCurTime());// << std::endl;


            if(!iLast.empty() && !iTrans.empty())
            {
                cv::Mat iTemp = iTrans * iLast.inv();
                //watcher(iLast, iTrans);
                std::cout << iTemp.at<float>(0, 3) << " " << iTemp.at<float>(1, 3) << " " << iTemp.at<float>(2, 3) << std::endl;
            }
            else
            {
                std::cout << "map reseting" << std::endl;
            }
            iLast = iTrans;
           //cv::waitKey(20);
